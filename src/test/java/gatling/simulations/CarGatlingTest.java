...
public class CarGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all cars").get("/services/crud/api/cars").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new car")
                        .post("/services/crud/api/cars")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"year\": 0" +
                                ", \"licensePlate\": \"SAMPLE_TEXT\"" +
                                ", \"model\": \"SAMPLE_TEXT\"" +
                                ", \"color\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created car").get("/services/crud${new_car_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created car").delete("/services/crud${new_car_url}").headers(headers_http_authenticated))
                .pause(10)
        );
