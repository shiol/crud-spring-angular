 ...
 */
public class UserGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all users").get("/services/crud/api/users").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new user")
                        .post("/services/crud/api/users")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created user").get("/services/crud${new_user_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created user").delete("/services/crud${new_user_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the User entity").exec(scn);
